...
D_promice = (vel_promice * th_ts).apply(
    lambda c: c * (200 * meta['err_2D'].values), axis=0
) * 917 / 1E12

D_promice_err = vel_promice.apply(
    lambda c: c * (th['fit_err'] * 200 * meta['err_2D'].values), axis=0
) * 917 / 1E12
...
for g in D_promice_gates.index:
    ...
    D_promice_gates_fill_weight.loc[g] = ((D_promice[g_idx] * fill_promice[g_idx]) /
                                          D_promice[g_idx].sum()).sum()
...
for s in D_promice_sectors.index:
    s_idx = DD_promice['name'] == s
    D_promice_sectors_fill_weight.loc[s] = ((D_promice[s_idx] * fill_promice[s_idx]) /
                                            D_promice[s_idx].sum()).sum()
...
for r in D_promice_regions.index:
    r_idx = DD_promice['regions'] == r
    D_promice_regions_fill_weight.loc[r] = ((D_promice[r_idx] * fill_promice[r_idx]) /
                                            D_promice[r_idx].sum()).sum()

# PROMICE :: Same, but all GIS
# D_promice_all :: PROMICE-only discharge sum
# D_promice_all_err :: PROMICE-only discharge error
# D_promice_all_fill :: PROMICE-only fill weighting
...
# Weighted fill using PROMICE discharge and PROMICE fill mask
D_promice_all_fill_weight = pd.Series(dtype=np.float64).reindex_like(D_promice_all)
for c in D_promice.columns:
    D_promice_all_fill_weight.loc[c] = (fill_promice[c] * 
                                       (D_promice[c] / D_promice[c].sum())).sum()
