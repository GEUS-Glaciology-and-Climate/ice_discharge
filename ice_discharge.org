...
| 2018-01-01 00:00:00 | 498.124 | 50.7055 |
| 2019-01-01 00:00:00 | 498.66  | 51.0551 |
| 2020-01-01 00:00:00 | 506.189 | 52.0592 |
:END:
...
SE %:  31.631849590920368
NE,NO,NW %: 31.454893067281176
...
2019-01-01    109.598467
2020-01-01    109.663308
...
2019-01-01    36.010600
2020-01-01    32.331538
...
print("#+CAPTION: Statistics of pixels with and without valid thickness. Numbers represent speed (m yr^{-1}) except for the \"count\" row\label{tab_thick_adjust}.")
df.round(0).astype(np.int)
#+END_SRC

#+CALL: tab_thick_adjust[:session discharge :eval no-export]()

#+RESULTS:
:RESULTS:
#+LATEX_ATTR: :placement [!h]
#+CAPTION: Statistics of pixels with and without valid thickness. Numbers represent speed (m yr^{-1}) except for the "count" row\label{tab_thick_adjust}.
...
**** Set Header Args                                      :ignore:
:PROPERTIES:
:header-args:jupyter-python+: :eval no-export :session discharge
:END:

We assume ice thicknesses < 20 m are incorrect where ice speed is > 100 m yr^{-1}. Of src_jupyter-python{vel.shape[0]} {{{results(=5829=)}}} pixels, src_jupyter-python{(th['bad'] == False).sum()} {{{results(=5205=)}}} have valid thickness, and src_jupyter-python{th['bad'].sum()} {{{results(=624=)}}} (src_jupyter-python{np.round(th['bad'].sum()/vel.shape[0]*100).astype(np.int)} {{{results(=11=)}}} %) have invalid thickness. However, the speed at the locations of the invalid thicknesses is generally much less (and therefore the assumed thickness is less), and the influence on discharge is less than an average pixel with valid thickness (Table [[tab:thick_adjust]]).

**** Set Header Args                                      :ignore:
:PROPERTIES:
:header-args:jupyter-python+: :eval no-export :session thick_adj
:END:

When aggregating by gate, there are src_jupyter-python{th['gates'].unique().size} {{{results(=267=)}}} gates. Of these, src_jupyter-python{(th.groupby('gates').mean()['bad'] == 0).sum()} {{{results(=179=)}}} (src_jupyter-python{np.round((th.groupby('gates').mean()['bad'] == 0).sum()/th['gates'].unique().size*100).astype(np.int)} {{{results(=67=)}}} %) have no bad pixels and src_jupyter-python{(th.groupby('gates').mean()['bad'] > 0).sum()} {{{results(=88=)}}} (src_jupyter-python{np.round((th.groupby('gates').mean()['bad'] > 0).sum()/th['gates'].unique().size*100).astype(np.int)} {{{results(=33=)}}} %) have some bad pixels, src_jupyter-python{(th.groupby('gates').mean()['bad'] > 0.5).sum()} {{{results(=64=)}}} have > 50 % bad pixels, and src_jupyter-python{(th.groupby('gates').mean()['bad'] == 1).sum()} {{{results(=62=)}}} (src_jupyter-python{np.round((th.groupby('gates').mean()['bad'] == 1).sum()/th['gates'].unique().size*100).astype(np.int)} {{{results(=23=)}}} %) are all bad pixels.
...
| 2018-12-31 00:00:00 |    477.119 | 484.416 | 7.29695 |
| 2019-12-31 00:00:00 |    478.724 |  486.23 | 7.50581 |
| 2020-12-31 00:00:00 |    485.026 | 492.899 | 7.87309 |
...
2018-12-31  484.415917  487.662391  3.246474
2019-12-31  486.229754  489.639942  3.410188
2020-12-31  492.898651  496.462147  3.563496
#+end_example
...
ds.to_netcdf('./out/GIS.nc', mode='w')
#+END_SRC
...
ds.to_netcdf('./out/region.nc', mode='w')
#+END_SRC
...
ds.to_netcdf('./out/sector.nc', mode='w')
#+END_SRC
...
ds.to_netcdf('./out/gate.nc', mode='w')
#+END_SRC
...
|                       |       D |       x |            y |
|-----------------------+---------+---------+--------------|
| HELHEIMGLETSCHER      | 34.4688 |  304157 | -2.57653e+06 |
| JAKOBSHAVN_ISBRAE     | 32.2649 | -182123 |  -2.2672e+06 |
| KANGERLUSSUAQ         | 27.5777 |  487300 | -2.28998e+06 |
| ZACHARIAE_ISSTROM     | 15.9557 |  488407 | -1.10017e+06 |
| KOGE_BUGT_C           | 15.4499 |  184648 | -2.72185e+06 |
| NIOGHALVFJERDSFJORDEN |  13.431 |  439187 |  -1.0736e+06 |
| RINK_ISBRAE           | 12.5077 | -224240 | -1.97637e+06 |
| PETERMANN_GLETSCHER   | 11.9372 | -261888 |      -996839 |
...
2018  496.209979  50.469782
2019  498.644843  51.057553
2020  506.155137  52.048609
#+end_example
...
| 2018-12-31 00:00:00 | 496.349 | 50.4818 |
| 2019-12-31 00:00:00 | 498.645 | 51.0576 |
| 2020-12-31 00:00:00 | 506.155 | 52.0486 |
:END:
...
| 2018-12-31 00:00:00 | 149.589 | 17.4308 |
| 2019-12-31 00:00:00 | 155.627 | 18.3665 |
| 2020-12-31 00:00:00 | 160.745 | 18.8264 |
:END:
...
2018-12-31    30.137826
2019-12-31    31.210052
2020-12-31    31.758140
...
2019-12-31    109.648593
2020-12-31    109.701089
...
| 2019-12-31 00:00:00 | 35.9532 | 3.48251 |
| 2020-12-31 00:00:00 | 32.2649 | 3.15109 |
:END:
...
| 2018-11-01 | 79.227 | 78.982 | 30.348 | 27.821 | 113.331 | 150.998 | 18.508 |
| 2018-11-13 | 80.392 | 76.226 | 30.326 | 27.609 | 113.148 |  152.42 | 18.641 |
| 2018-11-25 | 78.608 |  75.01 | 30.185 | 27.819 | 113.606 | 152.202 | 18.975 |
| 2018-12-07 | 79.371 | 74.221 | 30.309 | 27.854 | 112.553 | 150.071 | 19.019 |
| 2018-12-15 | 80.623 |  75.11 | 30.393 | 27.837 | 112.745 | 150.125 | 19.526 |
| 2018-12-19 | 81.249 | 75.555 | 30.435 | 27.828 | 112.841 | 150.148 | 19.171 |
| 2018-12-31 | 79.866 | 75.378 | 30.362 | 27.948 | 112.953 | 151.239 | 19.575 |
| 2019-01-12 | 79.503 | 73.995 | 30.392 | 27.955 | 112.979 | 153.118 | 20.056 |
