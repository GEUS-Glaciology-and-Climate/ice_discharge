...
#+NAME: commit-hash
#+BEGIN_SRC emacs-lisp :exports results :tangle no :eval no-export
(shell-command-to-string "git describe --always --dirty='*'")
#+END_SRC
...
This version of this README generated from git commit: 
<<commit-hash()>>
...
ds.attrs["source"] = "git commit: " + subprocess.check_output(["git", "describe", "--always",  "--dirty=*"]).strip().decode('UTF-8')
