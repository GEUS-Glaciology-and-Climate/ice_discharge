...
mkdir -p out_promice500m
cp ./out/gate_meta.csv ./out_promice500m/gate_meta.csv
#+END_SRC
...
###
vel = pd.read_csv("./tmp/dat_100_5000.csv", usecols=(lambda c: ('vel_eff' in c)))
vel_promice = pd.read_csv("./tmp/dat_100_5000.csv", usecols=lambda col: ("vel_eff" in col) and col.endswith("@promice"))

all_t = vel.rename(columns=lambda c: dt.datetime(int(c[8:12]),
                                                 int(c[13:15]),
                                                 int(c[16:18]))).columns.unique()

# 'velocity source' as in promice, measures.X, measures.Y, etc.
...
for vs in vel_sources:
    ...
    if "promice" in vs.lower():
        vel_promice = vs_vel.copy()
        vel_navg_promice = (vs_vel != 0).replace({True:1, False:0})
        vel_fill_promice = vs_fill.copy().replace(np.nan,0)
...
vel_final_promice = vel_promice / vel_navg_promice
vel_final_promice = vel_final_promice.fillna(method='bfill', axis=1).fillna(method='ffill', axis=1)

vel = vel_final
fill = vel_fill; fill[fill > 1] = 1

vel_promice = vel_final_promice
fill_promice = vel_fill_promice; fill_promice[fill_promice > 1] =1
...
# Separate PROMICE error (matching PROMICE velocity timestamps)
err_promice = err.loc[:, err.columns.isin(vel_promice.columns)].copy()
...
# PROMICE discharge — using PROMICE velocity and error
D_promice = (vel_promice * th_ts).apply(lambda c: c * (200 * meta['err_2D'].values), axis=0) * 917 / 1E12
D_promice_err = vel_promice.apply( lambda c: c * (th['fit_err'] * 200 * meta['err_2D'].values), axis=0) * 917 / 1E12

# Attach metadata like we did for DD / DD_err
DD_promice, DD_promice_err = [_.copy() for _ in [D_promice, D_promice_err]]
DD_promice[['gates','sectors','regions','ones','name']] = meta[['gates','sectors','regions','ones','name']]
DD_promice_err[['gates','sectors','regions','ones','name']] = meta[['gates','sectors','regions','ones','name']]
...
# Weighted fill for PROMICE only output
# D_gate :: Same, but at the gate scale
# D_gate_err ::
# D_gate_fill ::
D_promice_gates = DD_promice.groupby('gates').sum().drop(['ones','sectors'], axis=1)
D_promice_gates_err = DD_promice_err.groupby('gates').sum().drop(['ones','sectors'], axis=1)
D_promice_gates_fill_weight = pd.DataFrame(dtype=np.float64).reindex_like(D_promice_gates)
for g in D_promice_gates.index:
    g_idx = (DD_promice['gates'] == g)
    D_promice_gates_fill_weight.loc[g] = ((D_promice[g_idx]*fill_promice[g_idx])/D_promice[g_idx].sum()).sum()

D_promice_gates.columns = D_promice_gates.columns.astype(str).astype('datetime64[ns]')
D_promice_gates_err.columns = D_promice_gates_err.columns.astype(str).astype('datetime64[ns]')
D_promice_gates_fill_weight.columns = D_promice_gates_fill_weight.columns.astype(str).astype('datetime64[ns]')
D_promice_gates_fill_weight.clip(lower=0, upper=1, inplace=True)
...
# D_sector_err ::
# D_sector_fill ::
D_promice_sectors = DD_promice.groupby('name').sum().drop(['ones','sectors','gates'], axis=1)
D_promice_sectors_err = DD_promice_err.groupby('name').sum().drop(['ones','sectors','gates'], axis=1)
D_promice_sectors_fill_weight = pd.DataFrame(dtype=np.float64).reindex_like(D_promice_sectors)
for s in D_promice_sectors.index:
    s_idx = (DD['name'] == s)
    D_promice_sectors_fill_weight.loc[s] = ((D_promice[s_idx]*fill_promice[s_idx])/D_promice[s_idx].sum()).sum()

D_promice_sectors.columns = D_promice_sectors.columns.astype(str).astype('datetime64[ns]')
D_promice_sectors_err.columns = D_promice_sectors_err.columns.astype(str).astype('datetime64[ns]')
D_promice_sectors_fill_weight.columns = D_promice_sectors_fill_weight.columns.astype(str).astype('datetime64[ns]')
D_promice_sectors_fill_weight.clip(lower=0, upper=1, inplace=True)


# D_region :: Same, but at Mouginot region scale
# D_region_err ::
# D_region_fill ::
D_promice_regions = DD_promice.groupby('regions').sum().drop(['ones','sectors','gates'], axis=1)
D_promice_regions_err = DD_promice_err.groupby('regions').sum().drop(['ones','sectors','gates'], axis=1)
D_promice_regions_fill_weight = pd.DataFrame(dtype=np.float64).reindex_like(D_promice_regions)
for r in D_promice_regions.index:
    r_idx = DD['regions'] == r
    D_promice_regions_fill_weight.loc[r] = ((D_promice[r_idx]*fill_promice[r_idx])/D_promice[r_idx].sum()).sum()
...
D_promice_regions.columns = D_promice_regions.columns.astype(str).astype('datetime64[ns]')
D_promice_regions_err.columns = D_promice_regions_err.columns.astype(str).astype('datetime64[ns]')
D_promice_regions_fill_weight.columns = D_promice_regions_fill_weight.columns.astype(str).astype('datetime64[ns]')
D_promice_regions_fill_weight.clip(lower=0, upper=1, inplace=True)


# D_all :: Same, but all GIS
# D_all_err ::
# D_all_fill ::
D_promice_all = DD_promice.drop(['regions','sectors','ones','name','gates'], axis=1).sum()
D_promice_all_err = DD_promice_err.drop(['regions','sectors','ones','name','gates'], axis=1).sum()
D_promice_all_fill_weight = pd.Series(dtype=np.float64).reindex_like(D_all)
for c in D.columns:
    D_promice_all_fill_weight.loc[c] = (fill_promice[c] * (D_promice[c] / D_promice[c].sum())).sum()
...
# PROMICE :: Apply trimming to all PROMICE discharge outputs
STARTDATE='2016'
D_promice = D_promice.T[STARTDATE:].T
D_promice_err = D_promice_err.T[STARTDATE:].T
fill_promice = fill_promice.T[STARTDATE:].T

D_promice_gates = D_promice_gates.T[STARTDATE:].T
D_promice_gates_err = D_promice_gates_err.T[STARTDATE:].T
D_promice_gates_fill_weight = D_promice_gates_fill_weight.T[STARTDATE:].T

D_promice_sectors = D_promice_sectors.T[STARTDATE:].T
D_promice_sectors_err = D_promice_sectors_err.T[STARTDATE:].T
D_promice_sectors_fill_weight = D_promice_sectors_fill_weight.T[STARTDATE:].T

D_promice_regions = D_promice_regions.T[STARTDATE:].T
D_promice_regions_err = D_promice_regions_err.T[STARTDATE:].T
D_promice_regions_fill_weight = D_promice_regions_fill_weight.T[STARTDATE:].T

D_promice_all = D_promice_all.T[STARTDATE:].T
D_promice_all_err = D_promice_all_err.T[STARTDATE:].T
D_promice_all_fill_weight = D_promice_all_fill_weight.T[STARTDATE:].T
...
, *** PROMICE only
Since this is a PROMICE data product, a PROMICE only equivalent dataset is produced in out_promice500m.
...
# PROMICE gate level output
D_promice_gatesT = D_promice_gates.T
D_promice_gates_errT = D_promice_gates_err.T
D_promice_gates_fill_weightT = D_promice_gates_fill_weight.T

# Set index name for consistency
D_promice_gatesT.index.name = "Date"
D_promice_gates_errT.index.name = "Date"
D_promice_gates_fill_weightT.index.name = "Date"

# Save files to the PROMICE output folder
D_promice_gatesT.to_csv('./out_promice500m/gate_D.csv')
D_promice_gates_errT.to_csv('./out_promice500m/gate_err.csv')
D_promice_gates_fill_weightT.to_csv('./out_promice500m/gate_coverage.csv')
#+END_SRC
...
# PROMICE only
D_promice_sectorsT = D_promice_sectors.T
D_promice_sectors_errT = D_promice_sectors_err.T
D_promice_sectors_fill_weightT = D_promice_sectors_fill_weight.T

D_promice_sectorsT.index.name = "Date"
D_promice_sectors_errT.index.name = "Date"
D_promice_sectors_fill_weightT.index.name = "Date"

# meta_sector applies to PROMICE the same way as total (if present)
# meta_sector.to_csv('./out_promice500m/sector_meta.csv')  # if you output meta files

D_promice_sectorsT.to_csv('./out_promice500m/sector_D.csv')
D_promice_sectors_errT.to_csv('./out_promice500m/sector_err.csv')
D_promice_sectors_fill_weightT.to_csv('./out_promice500m/sector_coverage.csv')
...
# PROMICE only
D_promice_regionsT = D_promice_regions.T
D_promice_regions_errT = D_promice_regions_err.T
D_promice_regions_fill_weightT = D_promice_regions_fill_weight.T

D_promice_regionsT.index.name = "Date"
D_promice_regions_errT.index.name = "Date"
D_promice_regions_fill_weightT.index.name = "Date"

# meta_region applies to PROMICE the same way as total
# meta_region.to_csv('./out_promice500m/region_meta.csv')

D_promice_regionsT.to_csv('./out_promice500m/region_D.csv')
D_promice_regions_errT.to_csv('./out_promice500m/region_err.csv')
D_promice_regions_fill_weightT.to_csv('./out_promice500m/region_coverage.csv')
#+END_SRC
...
D_promice_all.index.name = "Date"
D_promice_all_err.index.name = "Date"
D_promice_all_fill_weight.index.name = "Date"

D_promice_all.to_csv('./out_promice500m/GIS_D.csv', float_format='%.3f',  header=["Discharge [Gt yr-1]"])
D_promice_all_err.to_csv('./out_promice500m/GIS_err.csv', float_format='%.3f', header=["Discharge Error [Gt yr-1]"])
D_promice_all_fill_weight.to_csv('./out_promice500m/GIS_coverage.csv', float_format='%.3f', header=["Coverage [unit]"])
#+END_SRC
...
cp ./out/gates.kmz ./out_promice500m/gates_promice.kmz
cp ./out/gates.gpkg ./out_promice500m/gates_promice.gpkg
cp ./out/gates.geojson ./out_promice500m/gates_promice.geojson
#+END_SRC
...
df_D = pd.read_csv('./out_promice500m/' + csvfile + '_D.csv', index_col=0, parse_dates=True)
df_err = pd.read_csv('./out_promice500m/' + csvfile + '_err.csv', index_col=0, parse_dates=True)
df_coverage = pd.read_csv('./out_promice500m/' + csvfile + '_coverage.csv', index_col=0, parse_dates=True)
...
ds.attrs["featureType"] = "timeSeries"
ds.attrs["title"] = "Greenland discharge using only PROMICE ice velocities"
ds.attrs["summary"] = "Greenland discharge"
ds.attrs["keywords"] = "Greenland; Ice Discharge; Calving; Submarine Melt"
# ds.attrs["Conventions"] = "CF-1.8"
ds.attrs["source"] = "git commit: " + subprocess.check_output(["git", "describe", "--always"]).strip().decode('UTF-8')
...
# ds.attrs["license"] = "TODO"
# ds.attrs["date_created"] = datetime.datetime.now().strftime("%Y-%m-%d")
ds.attrs["creator_name"] = "Signe Hillerup Larsen"
ds.attrs["creator_email"] = "shl@geus.dk"
ds.attrs["creator_url"] = "geus.dk"
ds.attrs["institution"] = "GEUS"
# ds.attrs["time_coverage_start"] = "TODO"
# ds.attrs["time_coverage_end"] = "TODO"
# ds.attrs["time_coverage_resolution"] = "TODO"
...
ds.to_netcdf('./out_promice500m/GIS.nc', mode='w')#, encoding=encoding)
#+end_src
...
ds.to_netcdf('./out_promice500m/region.nc', mode='w', encoding=encoding)
#+end_src
...
ds.attrs["featureType"] = "timeSeries"
ds.attrs["title"] = "Greenland discharge using PROMICE ice velocity only"
ds.attrs["summary"] = "Greenland discharge per sector"
ds.attrs["keywords"] = "Greenland; Ice Discharge; Calving; Submarine Melt"
# ds.attrs["Conventions"] = "CF-1.8"
ds.attrs["source"] = "git commit: " + subprocess.check_output(["git", "describe", "--always"]).strip().decode('UTF-8')
...
ds.to_netcdf('./out_promice500m/sector.nc', mode='w', encoding=encoding)
#+end_src
...
ds.to_netcdf('./out_promice500m/gate.nc', mode='w', encoding=encoding)
#+end_src
...
./IV_ROTR_20240721_20240813.nc
./IV_ROTR_20240802_20240825.nc
./IV_ROTR_20240826_20240918.nc
./IV_ROTR_20240907_20240930.nc
./IV_ROTR_20240919_20241012.nc
./IV_ROTR_20241001_20241024.nc
./IV_ROTR_20241130_20241223.nc
./IV_ROTR_20241212_20250104.nc
./IV_ROTR_20241224_20250116.nc
./IV_ROTR_20250105_20250128.nc
./IV_ROTR_20250117_20250209.nc
./IV_ROTR_20250129_20250221.nc
./IV_ROTR_20250209_20250305.nc
./IV_ROTR_20250221_20250317.nc
./IV_ROTR_20250305_20250329.nc
./IV_ROTR_20250319_20250410.nc
./IV_ROTR_20250329_20250422.nc
./IV_ROTR_20250410_20250504.nc
./IV_ROTR_20250422_20250516.nc
./IV_ROTR_20250504_20250528.nc
./IV_ROTR_20250516_20250609.nc
./IV_ROTR_20250528_20250621.nc
./IV_ROTR_20250609_20250703.nc
./IV_ROTR_20250621_20250715.nc
./IV_ROTR_20250703_20250727.nc
./IV_ROTR_20250715_20250808.nc
./IV_ROTR_20250727_20250820.nc
./IV_ROTR_20250808_20250901.nc
./IV_ROTR_20250820_20250913.nc
./IV_ROTR_20250901_20250925.nc
./IV_ROTR_20250913_20251007.nc
./IV_ROTR_20250925_20251019.nc
./IV_ROTR_20251007_20251031.nc
...
| 3c8309fe741865c54c334f56551e2fa0 | ./IV_ROTR_20250422_20250516.nc |
| b1ef8b4f03036c20e6a7356ad87045cb | ./IV_ROTR_20250504_20250528.nc |
| 925950d904cab0372e81df85ebe5421f | ./IV_ROTR_20250516_20250609.nc |
| 0e2240d674952f5fa6392436fb265b96 | ./IV_ROTR_20250528_20250621.nc |
| 1df0cf2c1b5f4d55d85daf9b097a642c | ./IV_ROTR_20250609_20250703.nc |
| 953216d7580c5ad455d39caba6ff87b2 | ./IV_ROTR_20250621_20250715.nc |
| e6d69a2083ddb6139b0274311dd87c05 | ./IV_ROTR_20250703_20250727.nc |
