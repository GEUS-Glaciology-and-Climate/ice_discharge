...
** Docker
:PROPERTIES:
:header-args:docker: :tangle no :eval no
:END:
*** Dockerfile: GRASS

#+BEGIN_SRC docker :tangle docker/grass/.dockerignore :mkdirp docker/grass
,*
#+END_SRC

#+BEGIN_SRC docker :tangle docker/grass/Dockerfile
FROM ubuntu:20.04
...
cd docker/grass
docker build --build-arg UID=$(id -u) --build-arg GID=$(id -g) -t ice_discharge_grass .
