...
th_ts = pd.DataFrame(dtype=np.float64).reindex_like(dem_ts)
th_ts['2020-08-01'] = dem_ts['202-08-01']
for idx in th.index:
    t0_pix = th.loc[idx]
    ...
    dem_pix = dem_ts.loc[idx]
    dem_pix = dem_pix - dem_pix.loc[t0_pix['day']]
    th_ts.loc[idx] = t0_pix_th + dem_pix
