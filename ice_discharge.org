...
#+BEGIN_EXPORT LaTeX
\title{Greenland Ice Sheet solid ice discharge from 1986 through February 2020}
...
In this version the NSIDC 0478 ice velocity data citep:NSIDC_0478 have been updated from v2 to v2.1. These data are used for the baseline velocity and gate selection, and therefore gate locations have shifted slightly, and number of gate pixels and number of gates have changed. In this version there are now src_bash[:eval yes]{tail -n +2 tmp/dat_100_5000.csv | wc -l} {{{results(=5829=)}}} pixels and src_bash[:eval yes]{cut -d"," -f8 tmp/dat_100_5000.csv | tail -n +2 | sort | uniq | wc -l} {{{results(=267=)}}} gates. Many gates remain the same pixel-for-pixel, but some do not meet the same inclusion criteria applied to this new baseline data, while some glaciers are included that were not. Many gates are also a few pixels narrower. Overall there are ~3 % fewer pixels, but only 8 fewer gates, and some of the gate reduction is due to combining two gates with a 1-pixel gap in the previous version. The effect of these changes is < 2.5 % of the estimated discharge.
